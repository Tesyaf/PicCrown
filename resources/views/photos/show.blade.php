...
<div class="min-h-screen py-16 px-6 bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-100 flex justify-center">
  <div class="max-w-3xl w-full space-y-10">
    ...
    <!-- Foto Detail -->
    <div class="backdrop-blur-xl bg-white/40 border border-white/20 rounded-3xl shadow-2xl p-8 hover:shadow-3xl transition-all duration-300">
      ...
      <div class="flex items-center justify-between border-t border-gray-300/40 pt-4">
        ...
        @if (auth()->id() === $photo->user_id)
          <div class="flex items-center gap-3">
            ...
            <form action="{{ route('photos.destroy', $photo->id) }}" method="POST"
                  onsubmit="return confirm('Yakin ingin menghapus foto ini?')" class="inline">
              @csrf
              @method('DELETE')
              <button type="submit"
    ...
    <!-- Komentar -->
    <div class="backdrop-blur-xl bg-white/30 border border-white/20 rounded-3xl shadow-xl p-8">
      <h2 class="text-2xl font-semibold text-gray-900 mb-6">Komentar & Penilaian</h2>

      @forelse ($mainComments as $comment)
        <div x-data="{ showReply: false, showEdit: false, openMenu: false }" class="mb-8">
          <div class="p-4 bg-white/60 rounded-xl shadow-inner border border-amber-100/50">
            <div class="flex justify-between items-start">
              <div>
                ...
                @endif
                <p class="mt-2 text-gray-800">{{ $comment->encrypted_comment }}</p>
              </div>
              ...
              <div class="relative" x-data="{ openMenu: false }">
                ...
                >
                  ...
                  @if (Auth::id() === $comment->user_id)
                    ...
                    <form action="{{ route('ratings.destroy', $comment->id) }}" method="POST" onsubmit="return confirm('Hapus komentar ini?')">
                      @csrf @method('DELETE')
            ...
            <!-- Edit Form -->
            <form x-show="showEdit" x-transition action="{{ route('ratings.update', $comment->id) }}" method="POST" class="mt-4 space-y-3">
              @csrf @method('PUT')
              <textarea name="comment" rows="2" class="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-orange-400">{{ $comment->encrypted_comment }}</textarea>
            ...
            <!-- Reply Form -->
            <form x-show="showReply" x-transition action="{{ route('ratings.store', $photo->id) }}" method="POST" class="mt-4 space-y-3">
              @csrf
              <input type="hidden" name="parent_id" value="{{ $comment->id }}">
              <textarea name="comment" rows="2" placeholder="Tulis balasan..." class="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-orange-400"></textarea>
      ...
      <div class="mt-10 border-t border-gray-300/30 pt-6">
        ...
        <form action="{{ route('ratings.store', $photo->id) }}" method="POST" class="space-y-5">
          @csrf
          <!-- Star Rating -->
          <div x-data="{ rating: 0 }" class="flex items-center gap-2">
            <template x-for="i in 5">
              <svg @click="rating = i" :class="i <= rating ? 'text-orange-500' : 'text-gray-300'"
          ...
          <textarea name="comment" rows="3" placeholder="Tulis komentar Anda..."
            class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-orange-400 outline-none resize-none backdrop-blur-md bg-white/50 text-gray-800 placeholder-gray-500 transition">
          </textarea>
